...
// Same reasoning as recommend-recipe.ts: this catalog is large (~10K tokens)
// and identical on every request, so it belongs in a cached system block —
// only the conversation itself (genuinely different per request) sits
