...
// This never changes between requests (same 112-recipe catalog every time),
// so it lives in `system` with a cache breakpoint rather than being
// re-sent as part of the per-request user message — repeat requests within
// the 5-minute TTL pay ~0.1x for these ~6,000 tokens instead of full price.
// Only the photo itself (genuinely different every request) stays uncached
// in the user message.
const SYSTEM_PROMPT =
