...
// Radius of the local-mean high-pass filter used ONLY for correlation-length
// measurement (not amplitude) — must be wider than the blob size it's meant
// to resolve, or the filter itself destroys the very structure being
// measured before autocorrelation ever sees it. A first pass at radius=1
// (3x3) produced an identical 1px correlation length for every one of the 4
// settings, including both grain SIZE options — a strong signal the filter
// was too narrow to resolve anything beyond immediate-neighbor noise, not
// that blob size genuinely doesn't vary (there's no documented hardware
// caveat for grain size the way there is for WB mode in patchRawProfile.ts).
const HIGHPASS_RADIUS_FOR_CORRELATION = 5;

/** Local-mean high-pass residual over a block, with a wide-enough kernel to preserve real blob structure. */
function highpassResidualRows(pixels, width, block, radius) {
  const rows = [];
  const windowSize = (radius * 2 + 1) ** 2;
  for (let y = block.y0 + radius; y < block.y1 - radius; y++) {
    const row = [];
    for (let x = block.x0 + radius; x < block.x1 - radius; x++) {
      let s = 0;
      for (let dy = -radius; dy <= radius; dy++) {
        for (let dx = -radius; dx <= radius; dx++) s += pixels[(y + dy) * width + (x + dx)];
      }
      row.push(pixels[y * width + x] - s / windowSize);
    }
    rows.push(row);
  }
  return rows;
...
/** Horizontal autocorrelation of a set of residual rows, at a given lag. */
function autocorrelation(rows, lag) {
  let sum = 0;
  let sumSq = 0;
  let count = 0;
...
function correlationLength(pixels, width, block) {
  const rows = highpassResidualRows(pixels, width, block, HIGHPASS_RADIUS_FOR_CORRELATION);
  for (let lag = 1; lag <= AUTOCORR_MAX_LAG; lag++) {
    if (autocorrelation(rows, lag) < 1 / Math.E) return lag;
  }
...
async function main() {
  ...
  if (smallCorrLengths.length > 0 && largeCorrLengths.length > 0) {
    ...
    if (avgSmall === avgLarge) {
      // Measured an identical correlation length for both sizes — the
      // measurement failed to discriminate them (there's no known hardware
      // reason grain size wouldn't apply), not evidence they're really the
      // same. Applying a 1:1 "correction" here would silently erase
      // today's existing Small/Large visual distinction, which is worse
      // than just keeping the untouched default — so keep the default
      // ratio instead of trusting this result.
      console.log(
        `\nMeasured Large/Small blob size identically (${avgSmall}px both) — the measurement couldn't ` +
          "discriminate them, keeping today's default sizeScale ratio instead of erasing the distinction.",
      );
    } else {
  ...
  const sizeScaleIsCalibrated = sizeScaleLarge !== DEFAULT_SIZE_SCALE_LARGE;
...
// is a real calibrated measurement. sizeScale.Large is ${
    sizeScaleIsCalibrated
      ? "calibrated RELATIVE to sizeScale.Small (see the script's header comment for why the absolute scale can't be derived the same way)."
      : "still today's uncalibrated default — the last run's blob-size measurement couldn't discriminate Small from Large (identical correlation length), so the default was kept rather than erasing the distinction with an untrustworthy 1:1 result."
  }
